...
summary: Décrit comment utiliser SUSE Linux Enterprise Server 15 SP6 avec ONTAP 
---
= Utilisez SUSE Linux Enterprise Server 15 SP6 avec ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Vous pouvez utiliser les paramètres de configuration de l'hôte SAN ONTAP pour configurer SUSE Linux Enterprise Server 15 SP6 avec ONTAP comme cible.
...
Votre configuration prend en charge le démarrage SAN. Reportez-vous au link:https://mysupport.netapp.com/matrix/imt.jsp?components=84067;&solution=1&isHWU&src=IMT["Matrice d'interopérabilité NetApp"^]pour vérifier que votre système d'exploitation, votre adaptateur de bus hôte, le micrologiciel de l'adaptateur de bus hôte, le BIOS de démarrage de l'adaptateur de bus hôte et la version de ONTAP sont pris en charge.
...
Pour SUSE Linux Enterprise Server 15 SP6, le `/etc/multipath.conf` fichier doit exister. Vous n'avez pas besoin d'apporter de modifications à ce fichier car SUSE Linux Enterprise Server 15 SP6 est compilé avec tous les paramètres requis pour reconnaître et gérer correctement les LUN ONTAP.
...
Le système d'exploitation SUSE Linux Enterprise Server 15 SP6 reconnaît les LUN ONTAP et définit automatiquement tous les paramètres de configuration pour les configurations ASA et non ASA. Les paramètres recommandés suivants vous permettent d'optimiser davantage les performances de votre configuration hôte.
...
----
.Exemple
Dans l'exemple suivant, vous déterminez le WWID d'un périphérique et vous ajoutez-le au `multipath.conf` fichier.
