...
---
= Configuration hôte NVMe-of pour RHEL 9.6 avec ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
----
. Sur l'hôte RHEL 9.6, vérifiez la chaîne hostnqn à l'adresse `/etc/nvme/hostnqn`:
+
[source, cli]
----
...
Avec RHEL 9.6, le  `nvmefc-boot-connections.service` et  `nvmf-autoconnect.service` services de démarrage inclus dans NVMe/FC  `nvme-cli` Les packages sont automatiquement activés pour démarrer au démarrage du système. Une fois le démarrage terminé, vérifiez que les services de démarrage sont activés.
