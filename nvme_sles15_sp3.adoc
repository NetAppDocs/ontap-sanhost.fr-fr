...
Pour passer confortablement aux événements de basculement ONTAP tels que les SFO, il est conseillé de définir une période de tentatives plus longue, comme par exemple 30 minutes, en ajustant le `ctrl_loss_tmo` minuterie. Voici les détails :
...
| `No discovery log entries to fetch` | Généralement vu si `/etc/nvme/hostnqn` Aucune chaîne n'a été ajoutée au sous-système correspondant de la baie NetApp ou une chaîne d'hôte nqn incorrecte a été ajoutée au sous-système respectif. | Assurez-vous que le système est exact `/etc/nvme/hostnqn` La chaîne est ajoutée au sous-système correspondant de la baie NetApp (vérifiez via le `vserver nvme subsystem host show` commande). 


| `Failed to write to /dev/nvme-fabrics: Operation already in progress` | Indique si les associations de contrôleur ou l'opération spécifiée sont déjà créées ou en cours de création. Cela peut se produire dans le cadre des scripts de connexion automatique installés ci-dessus. | Aucune. Pour découvrir nvme, essayez d'exécuter cette commande après un certain temps. Pour nvme connecter et se connecter-tous, exécutez `nvme list` commande pour vérifier que les périphériques d'espace de noms sont déjà créés et affichés sur l'hôte. 
|===
