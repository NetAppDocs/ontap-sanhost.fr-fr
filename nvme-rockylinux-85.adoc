...
---
= Configurer Rocky Linux 8.5 pour NVMe-oF avec stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Apprenez à configurer les hôtes NVMe over Fabrics (NVMe-oF) pour Rocky Linux 8.5. Pour plus d'assistance et d'informations sur les fonctionnalités, consultez link:nvme-rockylinux-supported-features.html["Prise en charge et fonctionnalités de Rocky Linux ONTAP"].

*NVMe-oF avec Rocky Linux 8.5 présente les limitations connues suivantes :*

* Le démarrage SAN à l'aide du protocole NVMe-oF n'est actuellement pas pris en charge.
* Le multipath NVMe dans le noyau est désactivé par défaut sur les hôtes NVMe-oF dans Rocky Linux 8.5 ; vous devez l'activer manuellement.
...
.Étapes
. Installez Rocky Linux 8.5 sur le serveur.  Une fois l'installation terminée, vérifiez que vous exécutez le noyau Rocky Linux 8.5 requis :
+
[source, cli]
----
uname -r
