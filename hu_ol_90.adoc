...
---
= Utilisez Oracle Linux 9.0 avec ONTAP
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
Vous pouvez utiliser les paramètres de configuration de l'hôte SAN ONTAP pour configurer Oracle Linux 9.0 avec ONTAP comme cible.
...
Les sections suivantes fournissent des exemples de sorties multivoies pour une LUN mappée aux rôles ASA et non ASA.
...
NOTE: Pour configurer Oracle Linux 9.0 Red Hat Enterprise Kernel (RHCK), utilisez link:hu_rhel_86.html#recommended-settings["paramètres recommandés"]pour Red Hat Enterprise Linux (RHEL) 9.0.
