...
Découvrez comment configurer les hôtes NVMe over Fabrics (NVMe-oF) pour RHEL 8.2.  Pour plus d'informations sur l'assistance et les fonctionnalités, consultezlink:hu-nvme-index.html["Présentation de NVME-oF"^] .
...
----
. Sur l'hôte RHEL 8.2, vérifiez la chaîne NQN de l'hôte à `/etc/nvme/hostnqn` :
+
[source, cli]
----
...
----
. Sur l'hôte RHEL 8.2, vérifiez la chaîne hostnqn à `/etc/nvme/hostnqn` :
+
[source, cli]
----
...
NOTE: Si les chaînes NQN de l'hôte ne correspondent pas, utilisez le `vserver modify` commande pour mettre à jour la chaîne NQN de l'hôte sur le sous-système de tableau ONTAP correspondant pour correspondre à la chaîne NQN de l'hôte à partir de `/etc/nvme/hostnqn` sur l'hôte.
