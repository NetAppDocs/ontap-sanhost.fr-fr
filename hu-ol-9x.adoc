...
summary: Installez les utilitaires hôtes Linux sur un hôte Oracle Linux 9.x pour gérer les opérations de protocole FCP et iSCSI avec les LUN ONTAP . 
---
= Configurer Oracle Linux 9.x pour FCP et iSCSI avec stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Le logiciel Linux Host Utilities fournit des outils de gestion et de diagnostic pour les hôtes Linux connectés au stockage ONTAP . Lorsque vous installez les utilitaires hôtes Linux sur un hôte Oracle Linux 9.x, vous pouvez utiliser ces utilitaires pour vous aider à gérer les opérations de protocole FCP et iSCSI avec les LUN ONTAP .
...
Vous pouvez utiliser le multipathing avec Oracle Linux 9.x pour gérer les LUN ONTAP .
...
== Étape 4 : Confirmez la configuration iSCSI de votre hôte
...
.Étapes
. Vérifiez que le paquet initiateur iSCSI (iscsi-initiator-utils) est installé :
+
[source, cli]
----
...
== Étape 5 : Facultativement, exclure un périphérique du multipathing
...
== Étape 6 : Personnaliser les paramètres multipath pour les LUNs ONTAP
...
Oracle Linux 9.3, 9.2, 9.1 et 9.0 avec stockage ONTAP présente le problème connu suivant :
...
| https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1508554["1508554"^] | L'utilitaire de LUN SAN avec HBA Emulex nécessite des liens symboliques à partir des modules de bibliothèque | Lorsque vous exécutez la commande CLI des utilitaires hôtes Linux « sanlun fcp show adapter -v » sur un hôte SAN, la commande échoue et affiche le message d'erreur suivant : « Les dépendances de bibliothèque requises pour la découverte d'un adaptateur de bus hôte (HBA) sont introuvables : ---- [root@hostname ~]# sanlun fcp show adapter -v Impossible de localiser la bibliothèque /usr/lib64/libHBAAPI.so. Assurez-vous que le paquet installant la bibliothèque est installé et chargé. » 
|===
