...
summary: Comment configurer un hôte NVMe-oF pour RHEL 9x avec ONTAP 
---
= Configurer RHEL 9x pour NVMe-oF avec stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Apprenez à configurer les hôtes NVMe over Fabrics (NVMe-oF) pour RHEL 9x. Pour plus d'assistance et d'informations sur les fonctionnalités, consultez link:nvme-rhel-supported-features.html["Prise en charge et fonctionnalités de RHEL ONTAP"].

NVMe-oF avec RHEL 9x présente les limitations connues suivantes :
...
.Étapes
. Installez RHEL 9x sur le serveur.  Une fois l'installation terminée, vérifiez que vous utilisez bien le noyau RHEL 9x requis :
+
[source, cli]
----
uname -r
...
----
. Recherchez la ligne qui définit la politique d’E/S pour le contrôleur NetApp ONTAP .
+
L'exemple suivant montre un exemple de règle :
...
NOTE: La nouvelle politique d'E/S s'applique automatiquement aux périphériques de contrôleur NetApp ONTAP correspondants.  Aucun redémarrage n'est requis.
...
L'authentification sécurisée en bande est prise en charge via NVMe/TCP entre un hôte RHEL 9x et un contrôleur ONTAP .
...
cat /etc/nvme/hostnqn
----
. Générez la clé dhchap pour l'hôte RHEL 9x.
+
