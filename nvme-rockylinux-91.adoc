...
summary: 'Comment configurer l"hôte NVMe-oF pour Rocky Linux 9.1 avec ONTAP' 
---
= Configurer Rocky Linux 9.1 avec NVMe-oF pour le stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Les hôtes Rocky Linux 9.1 prennent en charge les protocoles NVMe/FC et NVMe/TCP avec l'accès asymétrique aux espaces de noms (ANA). ANA est équivalent à l'accès asymétrique aux unités logiques (ALUA) dans les environnements iSCSI et FCP et est implémenté à l'aide de la fonctionnalité multichemin NVMe intégrée au noyau.
...
Vous pouvez utiliser le support et les fonctionnalités suivants avec la configuration d’hôte NVMe-oF pour Rocky Linux 9.1. Vous devez également vérifier les limites connues avant de commencer le processus de configuration.
...
+
** Rocky Linux 9.1 active le multipath NVMe dans le noyau pour les espaces de noms NVMe par défaut, supprimant ainsi le besoin de paramètres explicites.
...
Utilisez la procédure suivante pour valider les versions minimales du logiciel Rocky Linux 9.1 prises en charge.

.Étapes
. Installez Rocky Linux 9.1 sur le serveur. Une fois l'installation terminée, vérifiez que vous utilisez le noyau Rocky Linux 9.1 spécifié :
+
[source, cli]
----
uname -r
...
La configuration de l'hôte NVMe-oF pour Rocky Linux 9.1 avec stockage ONTAP présente le problème connu suivant :
...
| 1503468 | `nvme list-subsys` la commande renvoie des contrôleurs NVMe répétés répertoriés pour un sous-système donné | Le  `nvme list-subsys` La commande doit renvoyer une liste unique de contrôleurs NVMe associés à un sous-système donné. Sous Rocky Linux 9.1, la  `nvme list-subsys` La commande renvoie les contrôleurs NVMe avec leur état ANA respectif pour tous les espaces de noms appartenant à un sous-système donné. Cependant, l'état ANA étant un attribut propre à chaque espace de noms, des entrées de contrôleur NVMe uniques avec l'état du chemin doivent être affichées si vous répertoriez la syntaxe de commande du sous-système pour un espace de noms donné. 
|===