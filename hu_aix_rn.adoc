...
Les dernières versions des utilitaires hôtes AIX contiennent les nouvelles fonctionnalités et améliorations suivantes :
...
| S/O | Prise en charge de SCSI UNMAP pour les LUN NetApp ONTAP iSCSI | AIX Host Utilities 7.0 prend uniquement en charge la fonction SCSI UNMAP pour les périphériques FC. La prise en charge SCSI UNMAP pour les périphériques iSCSI n'est pas incluse dans cette version. 
