...
----
. Exécutez le `nvme connect-all` Contrôlez l'ensemble des LIF cible initiateur-initiateur NVMe/TCP prises en charge sur l'ensemble des nœuds. Assurez-vous de passer plus longtemps `ctrl_loss_tmo` période (par exemple, 30 minutes, qui peut être définie à l'aide de `-l 1800`) pendant le connect-all afin qu'il retente pendant une période plus longue en cas de perte de chemin. Par exemple :
+
[listing]
----
