...
[role="lead"]
Vous pouvez configurer NVMe over Fibre Channel (NVMe/FC) sur des hôtes initiateurs exécutant Oracle Linux 8.4 et ONTAP comme cible.
...
* Avec Oracle Linux 8.4, `nvme-fc auto-connect` des scripts sont inclus dans le package natif `nvme-cli`. Vous pouvez utiliser ces scripts de connexion automatique natifs au lieu d'installer les scripts de connexion automatique fournis par le fournisseur externe.
...
.Étapes
. Installez Oracle Linux 8.4 GA sur le serveur. Une fois l'installation terminée, vérifiez que vous exécutez le noyau Oracle Linux 8.4 GA spécifié. Voir link:https://mysupport.netapp.com/matrix/["Matrice d'interopérabilité"^]pour la liste actuelle des versions prises en charge.
+
[listing]
----
# uname -r
...
Le pilote natif de la boîte de réception qla2xxx inclus dans le noyau OL 8.4 GA possède les derniers correctifs en amont. Ces correctifs sont essentiels à la prise en charge de ONTAP.
...
La configuration hôte NVMe-of pour OL 8.4 avec ONTAP présente les problèmes connus suivants :
