...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/1508554["1508554"^] | L'interface de ligne de commande de NetApp Linux Host Utilities requiert d'autres dépendances au niveau des packages de bibliothèque pour prendre en charge la découverte d'adaptateurs HBA Emulex | Dans RHEL 9.1, la CLI des utilitaires hôtes SAN NetApp Linux `sanlun fcp show adapter -v` Échoue car les dépendances du package de bibliothèque pour prendre en charge la détection HBA sont introuvables. 
|===