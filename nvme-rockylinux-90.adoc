...
summary: 'Comment configurer l"hôte NVMe-oF pour Rocky Linux 9.0 avec ONTAP' 
---
= Configurer Rocky Linux 9.0 avec NVMe-oF pour le stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Les hôtes Rocky Linux 9.0 prennent en charge les protocoles NVMe/FC et NVMe/TCP avec l'accès asymétrique aux espaces de noms (ANA). ANA est équivalent à l'accès asymétrique aux unités logiques (ALUA) dans les environnements iSCSI et FCP et est implémenté à l'aide de la fonctionnalité multichemin NVMe intégrée au noyau.
...
Vous pouvez utiliser le support et les fonctionnalités suivants avec la configuration d’hôte NVMe-oF pour Rocky Linux 9.0. Vous devez également vérifier les limites connues avant de commencer le processus de configuration.
...
+
** Rocky Linux 9.0 active par défaut le multipath NVMe dans le noyau pour les espaces de noms NVMe, supprimant ainsi le besoin de paramètres explicites.
...
Utilisez la procédure suivante pour valider les versions minimales du logiciel Rocky Linux 9.0 prises en charge.

.Étapes
. Installez Rocky Linux 9.0 sur le serveur. Une fois l'installation terminée, vérifiez que vous utilisez le noyau Rocky Linux 9.0 spécifié :
+
[source, cli]
----
uname -r
...
La configuration de l'hôte NVMe-oF pour Rocky Linux 9.0 avec stockage ONTAP présente les problèmes connus suivants :
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | Les hôtes NVMe-oF de Rocky Linux 9.0 créent des contrôleurs de découverte persistante en double | Sur les hôtes NVMe over Fabrics (NVMe-of), vous pouvez utiliser la commande nvme Discover -p pour créer des contrôleurs de découverte persistants (CDP). Lorsque cette commande est utilisée, un seul PDC doit être créé par combinaison initiateur-cible. Cependant, si vous exécutez ONTAP 9.10.1 et Rocky Linux 9.0 avec un hôte NVMe-oF, un PDC en double est créé chaque fois que « nvme discover -p » est exécuté. Cela entraîne une utilisation inutile des ressources sur l'hôte et la cible. 
|===