...
summary: Installez les utilitaires hôtes Linux sur un hôte Proxmox VE 9.x pour gérer les opérations de protocole FCP et iSCSI avec les LUN ONTAP . 
---
= Configurer Proxmox VE 9.x pour FCP et iSCSI avec stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Configurez Proxmox VE 9.x pour le multipathing et avec des paramètres et réglages spécifiques pour les opérations de protocole FCP et iSCSI avec le stockage ONTAP .

FCP et iSCSI avec Proxmox VE 9.x présentent les limitations connues suivantes :

* Les utilitaires hôtes Linux ne prennent pas en charge les systèmes d'exploitation Proxmox VE 9.x.
...
Vous pouvez utiliser le multipathing avec Proxmox VE 9.x pour gérer les LUN ONTAP .
...
== Étape 2 : Confirmez la configuration iSCSI de votre hôte
