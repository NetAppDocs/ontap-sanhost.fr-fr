...
summary: 'Configuration d"hôte VME/FC pour Oracle Linux 8.6 avec ONTAP, par exemple' 
---
...
NVMe/FC est pris en charge sur ONTAP 9.6 ou version ultérieure pour Oracle Linux 8.6. L'hôte Oracle Linux 8.6 peut exécuter à la fois le trafic NVMe/FC et FCP via les mêmes ports d'adaptateur d'initiateur Fibre Channel (FC). Voir la link:https://hwu.netapp.com/["Hardware Universe"^] Pour obtenir la liste des contrôleurs et adaptateurs FC pris en charge, Pour obtenir la liste la plus récente des configurations prises en charge, consultez le link:https://mysupport.netapp.com/matrix/["Matrice d'interopérabilité NetApp"^] .
...
* Oracle Linux 8.6 a activé par défaut les chemins d'accès multiples NVMe dans le noyau pour les noms NVMe.
* Avec Oracle Linux 8.6, `nvme-fc auto-connect` des scripts sont inclus dans le stockage natif `nvme-cli` création de package. Vous pouvez donc vous appuyer sur ces scripts natifs de connexion automatique au lieu d'installer les scripts de connexion automatique fournis par le fournisseur externe.
* Avec Oracle Linux 8.6, une fonctionnalité native `udev` la règle est déjà fournie dans le cadre du `nvme-cli` Une solution qui permet l'équilibrage de la charge Round-Robin pour les chemins d'accès multiples NVMe. Vous n'avez donc plus besoin de créer cette règle manuellement (comme cela a été fait dans Oracle Linux 8.1).
...
* Avec Oracle Linux 8.6, le plug-in NetApp natif `nvme-cli` Est capable d'afficher les détails de ONTAP ainsi que les espaces de noms ONTAP.
...
* NVMe/TCP est une fonctionnalité d'aperçu de la technologie. Par conséquent, la prise en charge n'est pas publiée sur l'outil Interoperability Matrix Tool.
* Pour passer confortablement en cas d'événements de basculement ONTAP, tels que les basculements de stockage (SFO) pour NVMe/TCP, il est recommandé de définir une période de tentatives plus longue (par exemple, disons 30 minutes) en ajustant le `ctrl_loss_tmo` minuterie. Reportez-vous aux détails suivants.
...
== Activation de NVMe/FC avec Oracle Linux 8.6
...
----
. Sur l'hôte Oracle Linux 8.6, vérifiez le `hostnqn` chaîne à `/etc/nvme/hostnqn` et vérifier qu'il correspond au `hostnqn` Chaîne du sous-système correspondant sur la matrice ONTAP.
+
[listing]
----
...
NOTE: Si le `hostnqn` les chaînes ne correspondent pas, vous devez utiliser le `vserver modify` Commande pour mettre à jour la chaîne hostnqn sur le sous-système de tableau ONTAP correspondant à la valeur `hostnqn` chaîne de `/etc/nvme/hostnqn` sur l'hôte.

. Redémarrez l'hôte.
+
[NOTE]
====
Si vous prévoyez d'exécuter à la fois le trafic NVMe et SCSI sur le même hôte existant Oracle Linux 8.6, il est recommandé d'utiliser les chemins d'accès multiples NVMe in-kernel pour les espaces de noms ONTAP et `dm-multipath` Pour les LUN ONTAP respectivement. Et cela signifie également que les espaces de noms ONTAP devraient être noirci dans `dm-multipath` pour éviter `dm-multipath` en réclamant ces périphériques d'espace de noms. Pour ce faire, ajoutez le `enable_foreign` réglage sur `/etc/multipath.conf`:
...
----
====
. Redémarrez le démon multipathd en exécutant un `systemctl restart multipathd` commande permettant de laisser le nouveau paramètre prendre effet.
...
----
. Exécutez une commande dracut -f, puis redémarrez l'hôte.
...
.Étapes
. Le pilote natif qla2xxx inclus dans le noyau RHEL 8.6 GA est doté des derniers correctifs en amont, essentiels pour la prise en charge de ONTAP. Vérifiez que vous exécutez les versions du pilote de carte et du micrologiciel prises en charge :
+
[listing]
----
...
Pour passer confortablement aux événements de basculement ONTAP tels que les SFO, il est conseillé de définir une période de tentatives plus longue, comme, disons 30 minutes, en ajustant le `ctrl_loss_tmo` minuterie. Reportez-vous aux détails suivants.
...
...
----
. De la même manière, vérifiez que d'autres combos LIF cible-initiateur NVMe/TCP peuvent récupérer correctement les données de la page du journal de découverte. Par exemple :
+
[listing]
----
#nvme discover -t tcp -w 192.168.1.8 -a 192.168.1.51
...
round-robin
----
. Vérifier que les espaces de noms sont créés et découvreurs correctement sur l'hôte :
+
[listing]
----
# nvme list
...
Avant de commencer tout dépannage concernant une défaillance NVMe/FC, veillez à toujours exécuter une configuration conforme aux spécifications de IMT, puis suivez les étapes suivantes pour déboguer les problèmes côté hôte.
...
----
Vous pouvez donc régler le `lpfc_log_verbose` réglage du conducteur (ajouté à la ligne lpfc à `/etc/modprobe.d/lpfc.conf`) À n'importe laquelle des valeurs ci-dessus pour enregistrer des événements NVMe/FC du point de vue du pilote lpfc. Puis recréez les initiramfs en exécutant 'draut -f', puis redémarrez l'hôte. Après le redémarrage, vérifiez que la consignation détaillée s'applique en vérifiant les éléments suivants à l'aide des éléments ci-dessus `LOG_NVME_DISC` masque binaire comme exemple :
...
| `Failed to write to /dev/nvme-fabrics: Invalid argument` | Syntaxe incorrecte. | Assurez-vous d'utiliser la syntaxe correcte pour les commandes nvme. 


| `Failed to write to /dev/nvme-fabrics: No such file or directory` | Plusieurs problèmes peuvent déclencher cette action. Certains des cas courants sont : vous avez passé des arguments incorrects aux commandes nvme ci-dessus.  a| 
* Assurez-vous d'avoir réussi les arguments appropriés (comme la chaîne WWNN, la chaîne WWPN) pour les commandes ci-dessus.
* Si les arguments sont corrects, mais que cette erreur persiste, vérifiez si `/sys/class/scsi_host/host*/nvme_info` La sortie est correcte avec l'initiateur NVMe indiquant comme `Enabled` Les LIF cible NVMe/FC s'affichent correctement ici sous les sections des ports distants. Par exemple :
...
----
* Si les LIF cibles n'apparaissent pas comme ci-dessus dans la `nvme_info` sortie, vérifier le `/var/log/messages` et `dmesg` Sortie pour toute défaillance NVMe/FC suspecte et rapport/correction en conséquence.




| `No discovery log entries to fetch` | Ce message d'erreur s'affiche généralement si le `/etc/nvme/hostnqn` La chaîne n'a pas été ajoutée au sous-système correspondant de la baie NetApp. Ou une chaîne hostnqn incorrecte a été ajoutée au sous-système respectif. | Assurez-vous que le système est exact `/etc/nvme/hostnqn` La chaîne est ajoutée au sous-système correspondant de la baie NetApp (vérifiez via le `vserver nvme subsystem host show`). 


| `Failed to write to /dev/nvme-fabrics: Operation already in progress` | Ce message d'erreur s'affiche si les associations de contrôleur ou l'opération spécifiée sont déjà créées ou en cours de création. Cela peut se produire dans le cadre des scripts de connexion automatique installés ci-dessus. | Aucune. Pour `nvme discover`, essayez d'exécuter cette commande après un certain temps. Et peut-être pour `nvme connect` et `connect-all`, exécutez un `nvme list` commande pour vérifier que les périphériques d'espace de noms sont déjà créés et affichés sur l'hôte. 
|===
