...
summary: Décrit comment utiliser Oracle Linux 8.9 avec ONTAP 
---
= Utilisez Oracle Linux 8.9 avec ONTAP
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
Vous pouvez utiliser les paramètres de configuration de l'hôte SAN ONTAP pour configurer Oracle Linux 8.9 avec ONTAP comme cible.
...
.Ce dont vous avez besoin
Votre configuration prend en charge le démarrage SAN. Reportez-vous au link:https://mysupport.netapp.com/matrix/imt.jsp?components=91863;&solution=1&isHWU&src=IMT["Matrice d'interopérabilité NetApp"^]pour vérifier que votre système d'exploitation, votre adaptateur de bus hôte, le micrologiciel de l'adaptateur de bus hôte, le BIOS de démarrage de l'adaptateur de bus hôte et la version de ONTAP sont pris en charge.
...
Pour Oracle Linux (OL) 8.9, le `/etc/multipath.conf` fichier doit exister. Vous n'avez pas besoin d'apporter de modifications à ce fichier car Oracle Linux 8.9 est compilé avec tous les paramètres requis pour reconnaître et gérer correctement les LUN ONTAP.
...
NOTE: Une seule LUN ne doit pas avoir besoin de plus de quatre chemins. Plus de huit chemins peuvent entraîner des problèmes de chemin lors des défaillances du stockage.
...
Le système d'exploitation Oracle Linux 8.9 est compilé pour reconnaître les LUN ONTAP et définir automatiquement les paramètres de configuration corrects pour les configurations ASA et non ASA. Les paramètres recommandés suivants vous permettent d'optimiser davantage les performances de votre configuration hôte.
...
NOTE: Pour configurer Oracle Linux 8.9 RedHat Enterprise Kernel, utilisez link:hu_rhel_89.html#recommended-settings["paramètres recommandés"]pour Red Hat Enterprise Linux (RHEL) 8.9.



== Paramètres KVM

Vous pouvez également utiliser les paramètres recommandés pour configurer une machine virtuelle basée sur noyau (KVM). Aucune modification n'est requise pour configurer un KVM lorsque le LUN est mappé sur l'hyperviseur.



== Problèmes connus

Il n'y a aucun problème connu pour Oracle Linux 8.9 avec la version ONTAP.
