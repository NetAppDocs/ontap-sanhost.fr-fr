...
.Avant de commencer
Votre configuration prend en charge le démarrage SAN. Reportez-vous au link:https://imt.netapp.com/matrix/#welcome["Matrice d'interopérabilité"^]pour vérifier que votre système d'exploitation, votre adaptateur de bus hôte, le micrologiciel de l'adaptateur de bus hôte, le BIOS de démarrage de l'adaptateur de bus hôte et la version de ONTAP sont pris en charge.
