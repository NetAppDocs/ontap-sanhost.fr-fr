...
---
= Utilisation de Veritas Storage Foundation 6 pour Linux avec NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
Ce document fournit des conseils sur les paramètres de configuration des hôtes SAN ONTAP pour la version Veritas Storage Foundation 6 pour les plateformes Red Hat Enterprise Linux & Oracle Linux (basées sur RHCK) avec protocoles FC, FCoE et iSCSI.
...
Vous devez vérifier que votre configuration répond à la configuration système requise. Pour plus d'informations, reportez-vous à la section https://["Matrice d'interopérabilité NetApp"^] Et la matrice Veritas HCL.
