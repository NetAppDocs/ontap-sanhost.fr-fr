...
--
.Étapes
. Le pilote natif de la boîte de réception qla2xxx inclus dans le noyau OL 8.7 GA dispose des derniers correctifs en amont essentiels pour la prise en charge de ONTAP. Vérifiez que vous exécutez les versions du pilote de carte et du micrologiciel prises en charge :
+
[listing]
----
...
| 1517321 | Les hôtes Oracle Linux 8.7 NVMe-of créent des contrôleurs de détection persistante en double | Sur les hôtes OL 8.7 NVMe-of, les contrôleurs de découverte permanente (CDP) sont créés en transmettant le `-p` à la `nvme discover` commande. Pour une combinaison initiateur-cible donnée, un seul PDC doit être créé avec chaque appel du `nvme discover` commande. Toutefois, à partir de OL 8.x, les hôtes NVMe-of finissent par créer des CDP dupliqués avec chaque invocation du `nvme discover` commande avec `-p` option. Cela gaspille des ressources sur l'hôte et sur la cible. | https://bugzilla.oracle.com/bugzilla/show_bug.cgi?id=18118["18118"^] 
|===