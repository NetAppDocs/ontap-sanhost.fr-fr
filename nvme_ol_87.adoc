...
| 1517321 | Les hôtes Oracle Linux 8.7 NVMe-of créent des contrôleurs de détection persistante en double | Sur les hôtes OL 8.7 NVMe-of, les contrôleurs de découverte permanente (CDP) sont créés en transmettant l' `-p`option à la `nvme discover` commande. Pour une combinaison initiateur-cible donnée, chaque exécution de la `nvme discover` commande doit créer un PDC. Toutefois, à partir de OL 8.x, les hôtes NVMe-of créent des CDP dupliqués. Cela gaspille des ressources sur l'hôte et sur la cible. | https://bugzilla.oracle.com/bugzilla/show_bug.cgi?id=18118["18118"^] 
|===