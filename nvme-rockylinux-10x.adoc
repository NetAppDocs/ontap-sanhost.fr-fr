...
summary: Comment configurer un hôte NVMe-oF pour Rocky Linux 10.x avec stockage ONTAP 
---
= Configurer Rocky Linux 10.x pour NVMe-oF avec stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Apprenez à configurer les hôtes NVMe over Fabrics (NVMe-oF) pour Rocky Linux 10.x. Pour plus d'assistance et d'informations sur les fonctionnalités, consultez link:nvme-rockylinux-supported-features.html["Prise en charge et fonctionnalités de Rocky Linux ONTAP"].

NVMe-oF avec Rocky Linux 10.x présente les limitations connues suivantes :
...
.Étapes
. Installez Rocky Linux 10.x sur le serveur.  Une fois l'installation terminée, vérifiez que vous utilisez bien le noyau Rocky Linux 10.x requis :
+
[source, cli]
----
uname -r
...
L'authentification sécurisée en bande est prise en charge via NVMe/TCP entre un hôte Rocky Linux 10.x et un contrôleur ONTAP .
...
cat /etc/nvme/hostnqn
----
. Générez la clé dhchap pour l'hôte Rocky Linux 10.x.
+
