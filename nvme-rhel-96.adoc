...
Découvrez comment configurer les hôtes NVMe over Fabrics (NVMe-oF) pour Red Hat Enterprise Linux (RHEL) 9.6.  Pour plus d'informations sur l'assistance et les fonctionnalités, consultezlink:hu-nvme-index.html["Présentation de NVME-oF"^] .

*NVMe-oF avec RHEL 9.6 présente les limitations connues suivantes :*
...
.Étapes
. Installez RHEL 9.6 sur le serveur.  Une fois l'installation terminée, vérifiez que vous exécutez le noyau RHEL 9.6 requis :
+
[source, cli]
----
uname -r
...
=== Modifier la politique io dans les règles udev

RHEL 9.6 définit la politique d'E/S par défaut pour NVMe-oF sur `round-robin` .  Si vous souhaitez modifier la politique d'io en `queue-depth` , modifiez le fichier de règles udev comme suit :
...
----
. Exécutez les commandes suivantes pour recharger les règles udev et appliquer les modifications :
+
[source, cli]
----
...
----
. Vérifiez la politique d’E/S actuelle pour votre sous-système.  Remplacez <sous-système> par le nom approprié, par exemple, `nvme-subsys0` .
+
[source, cli]
----
...
À partir d' ONTAP 9.12.1, l'authentification sécurisée en bande est prise en charge via NVMe/TCP entre un hôte RHEL 10.0 et un contrôleur ONTAP .
