...
summary: 'Comment configurer l"hôte NVMe-oF pour Rocky Linux 9.2 avec ONTAP' 
---
= Configurer Rocky Linux 9.2 avec NVMe-oF pour le stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Les hôtes Rocky Linux 9.2 prennent en charge les protocoles NVMe/FC et NVMe/TCP avec l'accès asymétrique aux espaces de noms (ANA). ANA est équivalent à l'accès asymétrique aux unités logiques (ALUA) dans les environnements iSCSI et FCP et est implémenté à l'aide de la fonctionnalité multichemin NVMe intégrée au noyau.
...
Vous pouvez utiliser le support et les fonctionnalités suivants avec la configuration d’hôte NVMe-oF pour Rocky Linux 9.2. Vous devez également vérifier les limites connues avant de commencer le processus de configuration.
...
+
** Rocky Linux 9.2 active le multipath NVMe dans le noyau pour les espaces de noms NVMe par défaut, supprimant ainsi le besoin de paramètres explicites.
...
Utilisez la procédure suivante pour valider les versions minimales du logiciel Rocky Linux 9.2 prises en charge.

.Étapes
. Installez Rocky Linux 9.2 sur le serveur. Une fois l'installation terminée, vérifiez que vous utilisez le noyau Rocky Linux 9.2 spécifié :
+
[source, cli]
----
uname -r
