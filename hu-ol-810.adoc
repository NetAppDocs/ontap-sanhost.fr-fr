...
NOTE: Pour configurer Oracle Linux 8.10 RedHat Enterprise Kernel, utilisez link:hu_rhel_810.html#recommended-settings["paramètres recommandés"]pour Red Hat Enterprise Linux (RHEL) 8.10.



== Paramètres KVM

Vous pouvez également utiliser <<recommended_settings_ol_810,paramètres recommandés>>pour configurer une machine virtuelle basée sur noyau (KVM). Aucune modification n'est requise pour configurer un KVM car le LUN est mappé sur l'hyperviseur.



== Problèmes connus

Il n'y a aucun problème connu pour Oracle Linux 8.10 avec la version ONTAP.
