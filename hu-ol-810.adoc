...
---
= Configurer Oracle Linux 8.10 pour FCP et iSCSI avec le stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Le logiciel Linux Host Utilities fournit des outils de gestion et de diagnostic pour les hôtes Linux connectés au stockage ONTAP. Lorsque vous installez les utilitaires d'hôte Linux sur un hôte Oracle Linux 8.10, vous pouvez utiliser les utilitaires d'hôte pour vous aider à gérer les opérations de protocole FCP et iSCSI avec les LUN ONTAP.
...
Vous pouvez utiliser les chemins d'accès multiples avec Oracle Linux 8.10 pour gérer les LUN ONTAP.


NOTE: Vous pouvez utiliser le link:hu_rhel_810.html#rhel-rhck["paramètres recommandés pour Red Hat Enterprise Linux (RHEL) 8.10"] pour configurer le noyau compatible Red Hat pour Oracle Linux 8.10.
