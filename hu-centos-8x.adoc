...
summary: Installez les utilitaires hôtes Linux sur un hôte CentOS 8.x pour gérer les opérations de protocole FCP et iSCSI avec les LUN ONTAP . 
---
= Configurer CentOS 8.x pour FCP et iSCSI pour le stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Le logiciel Linux Host Utilities fournit des outils de gestion et de diagnostic pour les hôtes Linux connectés au stockage ONTAP . Lorsque vous installez les utilitaires hôtes Linux sur un hôte CentOS 8.x, vous pouvez utiliser ces utilitaires pour vous aider à gérer les opérations des protocoles FCP et iSCSI avec les LUN ONTAP .
...
Vous pouvez utiliser le multipathing avec CentOS 8.x pour gérer les LUN ONTAP .
...
Pour les problèmes connus du noyau compatible CentOS Red Hat, consultez la link:hu-rhel-8x.html#step-7-review-the-known-issues["problèmes connus"] pour la série Red Hat Enterprise Linux (RHEL) 8.x.
...
* Découvrez la virtualisation CentOS Linux (KVM)
+
CentOS Linux peut servir d'hôte KVM. Cela vous permet d'exécuter plusieurs machines virtuelles sur un seul serveur physique grâce à la technologie de machine virtuelle basée sur le noyau Linux (KVM). L'hôte KVM ne nécessite pas de paramètres de configuration hôte explicites pour les LUN ONTAP .
