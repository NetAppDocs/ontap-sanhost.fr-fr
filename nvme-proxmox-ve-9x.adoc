...
summary: Comment configurer un hôte NVMe-oF pour Proxmox VE 9.0 avec stockage ONTAP 
---
= Configurer Proxmox VE 9.0 pour NVMe-oF avec stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
L'hôte Proxmox VE 9.0 prend en charge les protocoles NVMe over Fibre Channel (NVMe/FC) et NVMe over TCP (NVMe/TCP) avec Asymmetric Namespace Access (ANA). ANA fournit une fonctionnalité de multi-accès équivalente à l'accès aux unités logiques asymétriques (ALUA) dans les environnements iSCSI et FCP.

Apprenez à configurer les hôtes NVMe over Fabrics (NVMe-oF) pour Proxmox VE 9.0. Pour plus d'assistance et d'informations sur les fonctionnalités, consultez link:nvme-proxmox-supported-features.html["Prise en charge et fonctionnalités ONTAP"^].


NOTE: Proxmox VE 9.0 ne prend pas en charge la configuration de démarrage SAN pour NVMe-FC.



== Étape 1 : Vérifier la version du logiciel et la configuration NVMe
...
.Étapes
. Installez Proxmox VE 9.0 sur le serveur.  Une fois l'installation terminée, vérifiez que vous utilisez bien le noyau Proxmox VE 9.0 requis :
+
[source, cli]
----
uname -r
----
+
Exemple de version du noyau Proxmox VE 9.0 :
