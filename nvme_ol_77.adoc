...
NVMe/FC est pris en charge sur ONTAP 9.6 ou version ultérieure dans les versions suivantes d'Oracle Linux

* OL 7.7
+
L'hôte OL 7.7 peut exécuter à la fois le trafic NVMe et SCSI via les mêmes ports d'adaptateur d'initiateur Fibre Channel. Voir la link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] Pour obtenir la liste des contrôleurs et adaptateurs FC pris en charge,

+
Pour obtenir la liste la plus récente des configurations prises en charge, consultez le link:https://mysupport.netapp.com/matrix/["Matrice d'interopérabilité NetApp"^].
