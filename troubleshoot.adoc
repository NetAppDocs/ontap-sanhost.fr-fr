...
Avant de commencer le dépannage, vérifiez que vous utilisez une configuration conforme à la norme. https://mysupport.netapp.com/matrix/#welcome["Matrice d'interopérabilité"^] Consultez les spécifications, puis passez aux étapes suivantes pour déboguer les éventuels problèmes côté hôte.


NOTE: Les instructions de dépannage ne s'appliquent pas aux hôtes AIX, ESXi, Proxmox et Windows.
...
Si le problème persiste, veuillez rassembler les fichiers et les résultats des commandes suivants et nous contacter. link:mysupport.netapp.com["Support NetApp"^] pour un triage plus approfondi :
