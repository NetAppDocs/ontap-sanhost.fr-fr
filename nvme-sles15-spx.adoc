...
summary: Décrit comment configurer NVMe/FC pour SUSE Linux Enterprise Server 15 SPx avec ONTAP. 
---
...
Découvrez comment configurer les hôtes NVMe over Fabrics (NVMe-oF) pour SUSE Linux Enterprise Server 15 SPx. Pour plus d'assistance et d'informations sur les fonctionnalités, consultez link:nvme-sles-supported-features.html["Prise en charge et fonctionnalités ONTAP"].
...
----
====
.. Exportez la TLS PSK depuis le trousseau de clés du noyau. Cela garantit que la clé persiste lors d'un redémarrage de l'hôte :
+
[source, cli]
----
