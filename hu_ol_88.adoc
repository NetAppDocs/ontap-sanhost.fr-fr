...
summary: Décrit comment utiliser Oracle Linux 8.8 avec ONTAP 
---
= Utilisez Oracle Linux 8.8 avec ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
...
[role="lead"]
Vous pouvez utiliser les paramètres de configuration de l'hôte SAN ONTAP pour configurer Oracle Linux 8.8 avec ONTAP comme cible.
...
Pour Oracle Linux (OL) 8.8 `/etc/multipath.conf` le fichier doit exister, mais il n'est pas nécessaire d'apporter des modifications spécifiques au fichier. OL 8.8 est compilé avec tous les paramètres requis pour reconnaître et gérer correctement les LUN ONTAP.
...
Le système d'exploitation OL 8.8 est compilé pour reconnaître les LUN ONTAP et définir automatiquement tous les paramètres de configuration correctement pour les configurations ASA et non ASA. Les paramètres recommandés suivants vous permettent d'optimiser davantage les performances de votre configuration hôte.
...
NOTE: Pour configurer Oracle Linux 8.8 RedHat Enterprise Kernel, utilisez le link:hu_rhel_88.html#recommended-settings["paramètres recommandés"] Pour Red Hat Enterprise Linux (RHEL) 8.8.
...
Il n'y a aucun problème connu pour Oracle Linux 8.8 avec la version ONTAP.
