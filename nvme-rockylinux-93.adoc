...
---
= Configurer Rocky Linux 9.3 pour NVMe-oF avec stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Apprenez à configurer les hôtes NVMe over Fabrics (NVMe-oF) pour Rocky Linux 9.3. Pour plus d'assistance et d'informations sur les fonctionnalités, consultez link:nvme-rockylinux-supported-features.html["Prise en charge et fonctionnalités de Rocky Linux ONTAP"].

*NVMe-oF avec Rocky Linux 9.3 présente les limitations connues suivantes :*
...
.Étapes
. Installez Rocky Linux 9.3 sur le serveur.  Une fois l'installation terminée, vérifiez que vous exécutez le noyau Rocky Linux 9.3 requis :
+
[source, cli]
----
uname -r
