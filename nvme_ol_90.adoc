...
[role="lead"]
NVMe over Fabrics (NVMe-of), y compris NVMe/FC et d'autres moyens de transport, est pris en charge avec Oracle Linux 9.0 avec ANA (Asymmetric Namespace Access). Dans les environnements NVMe-of, ANA est l'équivalent des chemins d'accès multiples ALUA dans les environnements iSCSI et FC. Il est implémenté avec les chemins d'accès multiples NVMe intégrés au noyau.
...
.Étapes
. Installez Oracle Linux 9.0 GA sur le serveur. Une fois l'installation terminée, vérifiez que vous exécutez le noyau Oracle Linux 9.0 GA spécifié.
+
[listing]
----
# uname -r
...
.Étapes
. Vérifiez les paramètres NVMe/FC suivants sur l'hôte Oracle Linux 9.0 :
+
[listing]
----
...
Si vous rencontrez des problèmes avec votre configuration, la journalisation détaillée peut fournir des informations précieuses essentielles au dépannage.
