...
summary: Installez Linux Host Utilities sur un hôte Oracle Linux 7.0 pour gérer les opérations de protocole FCP et iSCSI avec les LUN ONTAP. 
---
= Configurer Oracle Linux 7.0 pour FCP et iSCSI avec le stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Le logiciel Linux Host Utilities fournit des outils de gestion et de diagnostic pour les hôtes Linux connectés au stockage ONTAP. Lorsque vous installez les utilitaires d'hôte Linux sur un hôte Oracle Linux 7.0, vous pouvez utiliser les utilitaires d'hôte pour vous aider à gérer les opérations de protocole FCP et iSCSI avec les LUN ONTAP.
...
Vous pouvez utiliser les chemins d'accès multiples avec Oracle Linux 7.0 pour gérer les LUN ONTAP. Oracle Linux 7.0 prend en charge Unbreakable Enterprise Kernel (UEK) R3 et UEK R4. Le système d'exploitation démarre avec le noyau UEK R3 par défaut


NOTE: Vous pouvez utiliser le link:hu_rhel_70.html#rhel-rhck["paramètres recommandés pour Red Hat Enterprise Linux (RHEL) 7.0"] pour configurer le noyau compatible Red Hat pour Oracle Linux 7.0.
...
L'hôte Oracle Linux 7.0 avec stockage ONTAP présente les problèmes connus suivants :
