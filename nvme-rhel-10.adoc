...
summary: 'Comment configurer l"hôte NVMe-oF pour RHEL 10.0 avec ONTAP' 
---
= Configurer RHEL 10.0 pour NVMe-oF avec stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Découvrez comment configurer les hôtes NVMe over Fabrics (NVMe-oF) pour RHEL 10.0.  Pour plus d'informations sur l'assistance et les fonctionnalités, consultezlink:hu-nvme-index.html["Présentation de NVME-oF"^] .

*NVMe-oF avec RHEL 10.0 présente les limitations connues suivantes :*
...
.Étapes
. Installez RHEL 10.0 sur le serveur.  Une fois l'installation terminée, vérifiez que vous exécutez le noyau RHEL 10.0 requis :
+
[source, cli]
----
uname -r
...
cat /etc/nvme/hostnqn
----
. Générez la clé dhchap pour l’hôte RHEL 10.0.
+
