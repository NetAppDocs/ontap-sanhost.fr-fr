...
summary: 'Comment configurer l"hôte NVMe-oF pour Rocky Linux 10.0 avec stockage ONTAP' 
---
= Configurer Rocky Linux 10.0 pour NVMe-oF avec stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Apprenez à configurer les hôtes NVMe over Fabrics (NVMe-oF) pour Rocky Linux 10.0. Pour plus d'assistance et d'informations sur les fonctionnalités, consultez link:nvme-rockylinux-supported-features.html["Prise en charge et fonctionnalités de Rocky Linux ONTAP"].

*NVMe-oF avec Rocky Linux 10.0 présente les limitations connues suivantes :*
...
.Étapes
. Installez Rocky Linux 10.0 sur le serveur.  Une fois l'installation terminée, vérifiez que vous exécutez le noyau Rocky Linux 10.0 requis :
+
[source, cli]
----
uname -r
...
À partir d' ONTAP 9.12.1, l'authentification sécurisée en bande est prise en charge via NVMe/TCP entre un hôte Rocky Linux 10.0 et un contrôleur ONTAP .
...
cat /etc/nvme/hostnqn
----
. Générez la clé dhchap pour l'hôte Rocky Linux 10.0.
+
