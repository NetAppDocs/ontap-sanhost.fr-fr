...
summary: 'Comment configurer l"hôte NVMe-oF pour Rocky Linux 10 avec stockage ONTAP' 
---
= Configurer Rocky Linux 10 pour NVMe-oF avec stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Directive non résolue dans <stdin> - include::_include/nvme/reuse-nvme-introduction.adoc[]

.Description de la tâche
Vous pouvez utiliser le support et les fonctionnalités suivants avec la configuration d’hôte NVMe-oF pour Rocky Linux 10. Vous devez également vérifier les limites connues avant de commencer le processus de configuration.
...
** À partir d'ONTAP 9.12.1, l'authentification intrabande sécurisée est prise en charge pour NVMe/TCP. Vous pouvez utiliser l’authentification sécurisée en bande pour NVMe/TCP avec Rocky Linux 10.
...
+
** À partir de Rocky Linux 10, le multipathing NVMe natif est toujours activé et la prise en charge du multipathing DM pour NVMe-oF n'est pas prise en charge.
...
Utilisez la procédure suivante pour valider les versions minimales du logiciel Rocky Linux 10 prises en charge.

.Étapes
. Installez Rocky Linux 10 sur le serveur. Une fois l'installation terminée, vérifiez que vous utilisez le noyau Rocky Linux 10 spécifié :
+
[source, cli]
----
uname -r
...
Directive non résolue dans <stdin> - include::_include/nvme/reuse_nvme_enabling_broadcom_1mb_size.adoc[]



== Étape 5 : Vérifier les services de démarrage NVMe

Avec Rocky Linux 10, le  `nvmefc-boot-connections.service` et  `nvmf-autoconnect.service` services de démarrage inclus dans NVMe/FC  `nvme-cli` les packages sont automatiquement activés au démarrage du système.
...
Directive non résolue dans <stdin> - include::_include/nvme/reuse-nvme-configure-nvme-tcp.adoc[]
...
Directive non résolue dans <stdin> - include::_include/nvme/reuse-nvme-validate-nvme-of.adoc[]
...
À partir d'ONTAP 9.12.1, l'authentification sécurisée en bande est prise en charge via NVMe/TCP entre un hôte Rocky Linux 10 et un contrôleur ONTAP.
...
cat /etc/nvme/hostnqn
----
. Générez la clé dhchap pour l'hôte Rocky Linux 10.
+
