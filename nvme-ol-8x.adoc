...
summary: Configuration hôte NVMe-oF pour Oracle Linux 8.x avec ONTAP 
---
= Configurer Oracle Linux 8.x avec NVMe-oF pour le stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Apprenez à configurer les hôtes NVMe over Fabrics (NVMe-oF) pour Oracle Linux 8.x. Pour plus d'assistance et d'informations sur les fonctionnalités, consultez link:nvme-ol-supported-features.html["Prise en charge et fonctionnalités d'Oracle Linux ONTAP"].

NVMe-oF avec Oracle Linux 8.x présente les limitations connues suivantes :

* Le démarrage SAN utilisant le protocole NVMe-oF n'est pas pris en charge.
* La prise en charge de l'utilitaire hôte sanlun de NetApp n'est pas disponible pour NVMe-oF sur un hôte Oracle Linux 8.x. Au lieu de cela, vous pouvez compter sur le plug-in NetApp inclus dans la version native `nvme-cli` package pour tous les transports NVMe-oF.
...
Utilisez la procédure suivante pour valider les versions logicielles minimales prises en charge par Oracle Linux 8.x.

.Étapes
. Installez Oracle Linux 8.x sur le serveur. Une fois l'installation terminée, vérifiez que vous utilisez bien le noyau Oracle Linux 8.x spécifié.
+
[source, cli]
----
uname -r
...
----
. Sur l'hôte Oracle Linux 8.x, vérifiez le `hostnqn` chaîne à `/etc/nvme/hostnqn` :
+
[source, cli]
----
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | Les hôtes Oracle Linux 8.x NVMe-oF créent des contrôleurs de découverte persistants (PDC) en double. | Toutefois, si vous exécutez Oracle Linux 8x avec un hôte NVMe-oF, un PDC dupliqué est créé à chaque exécution de `nvme discover -p`. Lorsque cette commande est utilisée, un seul PDC doit être créé par combinaison initiateur-cible. Toutefois, si vous utilisez Oracle Linux 8.x avec un hôte NVMe-oF, un PDC dupliqué est créé à chaque fois. `nvme discover -p` est exécuté. Cela entraîne une utilisation inutile des ressources, tant sur l'hôte que sur la cible. 
|===