...
Vous pouvez télécharger le fichier compressé contenant les progiciels Host Utilities à partir du link:https://mysupport.netapp.com/site/products/all/details/hostutilities/downloads-tab/download/61343/6.1/downloads["Site de support NetApp"^]. Une fois le fichier créé, vous devez le décompresser pour obtenir les deux packages logiciels dont vous avez besoin pour installer les utilitaires hôtes.

La version 6.1 d'AIX Host Utilities résout le problème de fuite de mémoire signalé dans les versions précédentes. Pour plus d'informations, reportez-vous aulink:hu_aix_rn.html["notes de version"].
