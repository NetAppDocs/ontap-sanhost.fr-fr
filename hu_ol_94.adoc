...
.Avant de commencer
Utilisez le link:https://mysupport.netapp.com/matrix/imt.jsp?components=91241;&solution=236&isHWU&src=IMT["Matrice d'interopérabilité NetApp"^] pour vérifier que votre système d'exploitation, votre adaptateur HBA, le micrologiciel HBA, le BIOS de démarrage HBA et la version ONTAP sont pris en charge.
...
NOTE: Pour configurer Oracle Linux 9.4 Red Hat Enterprise Kernel (RHEK), utilisez link:hu_rhel_94.html#recommended-settings["paramètres recommandés"]pour Red Hat Enterprise Linux (RHEL) 9.4.
