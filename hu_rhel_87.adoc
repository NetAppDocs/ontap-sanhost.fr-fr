...
summary: Décrit comment utiliser Red Hat Enterprise Linux 8.7 avec ONTAP 
---
= Utilisation de Red Hat Enterprise Linux 8.7 avec NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
Pour Red Hat Enterprise Linux (RHEL) 8.7 `/etc/multipath.conf` le fichier doit exister, mais il n'est pas nécessaire d'apporter des modifications spécifiques au fichier. RHEL 8.7 est compilé avec tous les paramètres requis pour reconnaître et gérer correctement les LUN ONTAP.
...
Le système d'exploitation RHEL 8.7 est compilé pour reconnaître les LUN ONTAP et définir automatiquement tous les paramètres de configuration pour les configurations ASA et non ASA.
...
| 1501048 | Paramètre d'expiration de la couche de transport FC `dev_loss_tmo` Pour les ports cible SCSI distants, indique une valeur négative (-1) |  `dev_loss_tmo` Le paramètre timeout représente le nombre de secondes par défaut que le transport FC doit isoler pour éviter la perte d'un port distant. Lorsque cette valeur est dépassée, la cible SCSI est supprimée de l'hôte. La valeur de ce paramètre doit être un entier positif. Cependant dans RHEL 8.7, `sysfs` le système de fichiers renvoie une valeur négative (-1) pour le `dev_loss_tmo` Paramètre de délai d'expiration pour les périphériques SCSI. Voici l'exemple de ligne de commande pour vérifier le `dev_loss_tmo` paramètre sur un hôte actif :
`# cat /sys/devices/pci*/*/*/host*/rport*/fc_remote_ports/rport*/dev_loss_tmo
4294967295`. Il n'existe aucune solution pour ce problème dans RHEL 8.7. | link:https://bugzilla.redhat.com/show_bug.cgi?id=2124297["2124297"^] 
|===
