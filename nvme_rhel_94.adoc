...
NVMe/TCP ne dispose pas de la fonctionnalité de connexion automatique. Par conséquent, nous devons exécuter manuellement la fonctionnalité de connexion NVMe/TCP ou de connexion pour découvrir les sous-systèmes et les espaces de noms NVMe/TCP.
...
NOTE: À partir de RHEL 9.4, le paramètre par défaut pour NVMe/TCP `ctrl_loss_tmo` le délai d'attente est désactivé, ce qui signifie qu'il n'y a pas de limite au nombre de tentatives (tentatives indéterminées). Par conséquent, vous n'avez pas besoin de configurer manuellement un spécifique `ctrl_loss_tmo` durée du délai d'attente lors de l'utilisation des commandes nvme connect ou nvme connect-all (option -l ). Avec ce comportement par défaut, les contrôleurs NVMe/TCP ne connaissent pas de temporisation en cas de défaillance sur un chemin et restent connectés indéfiniment.
