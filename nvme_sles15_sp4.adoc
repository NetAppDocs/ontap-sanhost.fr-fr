...
Pour basculer en toute sérénité les événements de basculement ONTAP tels que le basculement du stockage (SFO, Storage failover), il est recommandé de définir une période de tentatives plus longue, comme disons 30 minutes, en ajustant le `ctrl_loss_tmo` minuterie. Voici les détails :
