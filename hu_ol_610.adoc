...
summary: 'Décrit l"utilisation d"Oracle Linux 6.10 avec ONTAP' 
---
= Utilisez Oracle Linux 6.10 avec ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Vous pouvez utiliser les paramètres de configuration de l'hôte SAN ONTAP pour configurer Oracle Linux 6.10 avec ONTAP comme cible.
...
Pour Oracle Linux 6.10, le fichier /etc/multipath.conf doit exister, mais vous n'avez pas besoin d'apporter de modifications spécifiques au fichier. Oracle Linux 6.10 est compilé avec tous les paramètres requis pour reconnaître et gérer correctement les LUN ONTAP. Pour activer le gestionnaire ALUA, effectuez les opérations suivantes :
...
Oracle Linux 6.10 OS est compilé pour reconnaître les LUN ONTAP et définir automatiquement tous les paramètres de configuration correctement.
...
NOTE: Pour configurer Oracle Linux 6.10 Red Hat Enterprise Kernel (RHCK), utilisez link:hu_rhel_610.html#recommended-settings["paramètres recommandés"]pour Red Hat Enterprise Linux (RHEL) 6.10.



== Problèmes connus

Il n'y a aucun problème connu pour Oracle Linux 6.10 avec la version ONTAP.


NOTE: Pour les problèmes connus liés à Oracle Linux (noyau compatible Red Hat), consultez le link:hu_rhel_610.html#known-problems-and-limitations["problèmes connus"] Pour Red Hat Enterprise Linux (RHEL) 6.10.
