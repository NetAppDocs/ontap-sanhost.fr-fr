...
Découvrez comment configurer les hôtes NVMe over Fabrics (NVMe-oF) pour RHEL 8.7.  Pour plus d'informations sur l'assistance et les fonctionnalités, consultezlink:hu-nvme-index.html["Présentation de NVME-oF"^] .
...
.Étapes
. Installez RHEL 8.7 sur le serveur.  Une fois l'installation terminée, vérifiez que vous exécutez le noyau RHEL 8.7 requis :
+
[source, cli]
----
uname -r
...
----
. Sur l'hôte RHEL 8.7, vérifiez le `hostnqn` chaîne à `/etc/nvme/hostnqn` :
+
[source, cli]
----
