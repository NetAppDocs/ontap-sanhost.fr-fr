...
---
= Configurer RHEL 8.5 pour NVMe-oF avec stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Découvrez comment configurer les hôtes NVMe over Fabrics (NVMe-oF) pour RHEL 8.5.  Pour plus d'informations sur l'assistance et les fonctionnalités, consultezlink:hu-nvme-index.html["Présentation de NVME-oF"^] .

*NVMe-oF avec RHEL 8.5 présente les limitations connues suivantes :*

* Le démarrage SAN à l'aide du protocole NVMe-oF n'est actuellement pas pris en charge.
* Le multipath NVMe dans le noyau est désactivé par défaut sur les hôtes NVMe-oF dans RHEL 8.5 ; vous devez l'activer manuellement.
...
.Étapes
. Installez RHEL 8.5 sur le serveur.  Une fois l'installation terminée, vérifiez que vous exécutez le noyau RHEL 8.5 requis :
+
[source, cli]
----
uname -r
...
----
. Sur l'hôte RHEL 8.5, vérifiez le `hostnqn` chaîne à `/etc/nvme/hostnqn` :
+
[source, cli]
----
