...
---
= Configurer RHEL 9.4 pour NVMe-oF avec stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Découvrez comment configurer les hôtes NVMe over Fabrics (NVMe-oF) pour RHEL 9.4.  Pour plus d'informations sur l'assistance et les fonctionnalités, consultezlink:hu-nvme-index.html["Présentation de NVME-oF"^] .

NVMe-oF avec RHEL 9.4 n'a aucune limitation connue.
...
.Étapes
. Installez RHEL 9.4 sur le serveur.  Une fois l'installation terminée, vérifiez que vous exécutez le noyau RHEL 9.4 requis :
+
[source, cli]
----
uname -r
...
À partir d' ONTAP 9.12.1, l'authentification sécurisée en bande est prise en charge via NVMe/TCP entre un hôte RHEL 9.4 et un contrôleur ONTAP .
...
cat /etc/nvme/hostnqn
----
. Générez la clé dhchap pour l’hôte RHEL 9.4.
+
