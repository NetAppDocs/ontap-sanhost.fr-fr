...
summary: 'Comment configurer l"hôte NVMe-oF pour Rocky Linux 8.4 avec ONTAP' 
---
= Configurer Rocky Linux 8.4 avec NVMe-oF pour le stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Les hôtes Rocky Linux 8.4 prennent en charge les protocoles NVMe/FC et NVMe/TCP avec l'accès asymétrique aux espaces de noms (ANA). ANA est équivalent à l'accès asymétrique aux unités logiques (ALUA) dans les environnements iSCSI et FCP et est implémenté à l'aide de la fonctionnalité NVMe multipath intégrée au noyau.
...
Vous pouvez utiliser le support et les fonctionnalités suivants avec la configuration d’hôte NVMe-oF pour Rocky Linux 8.4. Vous devez également vérifier les limites connues avant de commencer le processus de configuration.
...
* Limitations connues :
+
** Le multipath NVMe dans le noyau est désactivé par défaut pour les hôtes Rocky Linux 8.4 NVMe-oF. Par conséquent, vous devez l'activer manuellement.
** Sur les hôtes Rocky Linux 8.4, NVMe/TCP est une fonctionnalité d'aperçu technologique en raison de problèmes ouverts. Reportez-vous au https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html-single/8.4_release_notes/index#technology-preview_file-systems-and-storage["Notes de version de Rocky Linux 8.4"^] pour plus de détails.
...
Utilisez la procédure suivante pour valider les versions minimales du logiciel Rocky Linux 8.4 prises en charge.

.Étapes
. Installez Rocky Linux 8.4 sur le serveur. Une fois l'installation terminée, vérifiez que vous utilisez le noyau Rocky Linux 8.4 spécifié :
+
[source, cli]
----
uname -r
...
Aucun problème connu n'existe pour la configuration de l'hôte NVMe-oF sur Rocky Linux 8.4 avec la version ONTAP.
