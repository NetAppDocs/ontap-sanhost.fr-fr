...
L'installation des utilitaires hôtes Linux ne modifie aucun paramètre ni comportement de votre hôte.
...
NOTE: Les utilitaires hôtes Linux ont une dépendance d'installation à la bibliothèque libHBAAPI.so. Vous devez installer l'un des paquets listés ci-dessus, même si un hôte est destiné à une configuration exclusivement iSCSI. Vous pouvez également utiliser des outils au niveau du système d'exploitation pour la gestion des LUN et des chemins d'accès.
