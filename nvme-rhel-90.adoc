...
---
= Configurer RHEL 9.0 pour NVMe-oF avec stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Découvrez comment configurer les hôtes NVMe over Fabrics (NVMe-oF) pour RHEL 9.0.  Pour plus d'informations sur l'assistance et les fonctionnalités, consultezlink:hu-nvme-index.html["Présentation de NVME-oF"^] .

*NVMe-oF avec RHEL 9.0 présente les limitations connues suivantes :*
...
.Étapes
. Installez RHEL 9.0 sur le serveur.  Une fois l'installation terminée, vérifiez que vous exécutez le noyau RHEL 9.0 requis :
+
[source, cli]
----
uname -r
...
----
. Sur l'hôte RHEL 9.0, vérifiez le `hostnqn` chaîne à `/etc/nvme/hostnqn` :
+
[source, cli]
----
