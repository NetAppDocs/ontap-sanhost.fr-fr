...
summary: Ce document explique comment utiliser Oracle Linux 8.x avec ONTAP. 
---
= Configurer Oracle Linux 8.x pour FCP et iSCSI avec stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Le logiciel Linux Host Utilities fournit des outils de gestion et de diagnostic pour les hôtes Linux connectés au stockage ONTAP . Lorsque vous installez les utilitaires hôtes Linux sur un hôte Oracle Linux 8.x, vous pouvez utiliser ces utilitaires pour vous aider à gérer les opérations des protocoles FCP et iSCSI avec les LUN ONTAP .
...
Vous pouvez utiliser le multipathing avec Oracle Linux 8.x pour gérer les LUN ONTAP .


NOTE: Vous pouvez utiliser le link:hu-rhel-8x.html#rhel-rhck["Paramètres recommandés pour Red Hat Enterprise Linux (RHEL) 8.x"] Configurer le noyau compatible Red Hat pour Oracle Linux 8.x.
