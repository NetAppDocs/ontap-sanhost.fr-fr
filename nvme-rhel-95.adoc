...
---
= Configurer RHEL 9.5 pour NVMe-oF avec stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Découvrez comment configurer les hôtes NVMe over Fabrics (NVMe-oF) pour Red Hat Enterprise Linux (RHEL) 9.5.  Pour plus d'informations sur l'assistance et les fonctionnalités, consultezlink:hu-nvme-index.html["Présentation de NVME-oF"^] .

NVMe-oF avec RHEL 9.5 n'a aucune limitation connue.
...
.Étapes
. Installez RHEL 9.5 sur le serveur.  Une fois l'installation terminée, vérifiez que vous exécutez le noyau RHEL 9.5 requis :
+
[source, cli]
----
uname -r
