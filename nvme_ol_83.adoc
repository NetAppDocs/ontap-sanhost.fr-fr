...
* Avec Oracle Linux 8.3, `nvme-fc auto-connect` les scripts sont inclus dans le package nvme-cli natif. Vous pouvez utiliser ces scripts de connexion automatique natifs au lieu d'installer les scripts de connexion automatique fournis par le fournisseur externe.
...
Reportez-vous au http://mysupport.netapp.com/matrix3["Matrice d'interopérabilité (IMT)"^] pour obtenir la liste actuelle des configurations prises en charge.
...
.Étapes
. Installez Oracle Linux 8.3 GA sur le serveur. Une fois l'installation terminée, vérifiez que vous exécutez le noyau Oracle Linux 8.3 GA spécifié. Voir https://mysupport.netapp.com/matrix/["Matrice d'interopérabilité"^] pour la liste actuelle des versions prises en charge.
+
[listing]
----
# uname -r
...
[NOTE]
====
Si vous avez l'intention d'exécuter à la fois le trafic NVMe et SCSI sur le même hôte Oracle Linux 8.3, NetApp vous recommande d'utiliser le chemin d'accès à distance NVMe intégré au noyau pour les namespaces ONTAP et `dm-multipath` pour les LUN ONTAP, respectivement. Cela signifie également que les espaces de noms ONTAP doivent être mis sur liste noire dans `dm-multipath` pour empêcher `dm-multipath` de réclamer ces périphériques d'espace de noms. Pour ce faire, ajoutez le paramètre _enable_Foreign_ au `/etc/multipath.conf` fichier :
...
.Étapes
. Vérifiez que vous utilisez la carte prise en charge. Pour obtenir la liste actuelle des cartes prises en charge, reportez-vous au https://mysupport.netapp.com/matrix/["Matrice d'interopérabilité"^] .
+
[listing]
----
...
Emulex LPe36002-M64 2-Port 64Gb Fibre Channel Adapter
----
. Vérifiez que vous utilisez le micrologiciel et le pilote recommandés pour la boîte de réception Broadcom lpfc. Pour obtenir la liste actuelle des pilotes de carte et des versions de micrologiciel pris en charge, consultez le https://mysupport.netapp.com/matrix/["Matrice d'interopérabilité"^].
+
[listing]
----
...
Le pilote natif de la boîte de réception qla2xxx inclus dans le noyau OL 8.3 GA possède les derniers correctifs en amont. Ces correctifs sont essentiels à la prise en charge de ONTAP.
