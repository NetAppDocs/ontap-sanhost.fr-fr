...
---
= Utilisation de SUSE Linux Enterprise Server 15 SP4 avec NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
Remplacer l' `<DevId>` avec le `WWID` chaîne du périphérique à exclure. Utiliser la commande suivante pour déterminer le `WWID`:
...
----
. Ajoutez-le `WWID` à la strophe de la liste noire de la `/etc/multipath.conf`:
+
[listing]
----
blacklist {
...
Il n'y a pas de problèmes connus pour SUSE Linux Enterprise Server 15 SP4.
