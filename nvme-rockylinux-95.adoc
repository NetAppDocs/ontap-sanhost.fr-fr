...
---
= Configurer Rocky Linux 9.5 pour NVMe-oF avec stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Apprenez à configurer les hôtes NVMe over Fabrics (NVMe-oF) pour Rocky Linux 9.5. Pour plus d'assistance et d'informations sur les fonctionnalités, consultez link:nvme-rockylinux-supported-features.html["Prise en charge et fonctionnalités de Rocky Linux ONTAP"].

NVMe-oF avec Rocky Linux 9.5 n'a aucune limitation connue.
...
.Étapes
. Installez Rocky Linux 9.5 sur le serveur.  Une fois l'installation terminée, vérifiez que vous exécutez le noyau Rocky Linux 9.5 requis :
+
[source, cli]
----
uname -r
