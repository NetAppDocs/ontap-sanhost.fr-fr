...
summary: 'Comment configurer l"hôte NVMe-oF pour Rocky Linux 9.5 avec ONTAP' 
---
= Configurer Rocky Linux 9.5 avec NVMe-oF pour le stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Les hôtes Rocky Linux 9.5 prennent en charge les protocoles NVMe/FC et NVMe/TCP avec l'accès asymétrique aux espaces de noms (ANA). ANA est équivalent à l'accès asymétrique aux unités logiques (ALUA) dans les environnements iSCSI et FCP et est implémenté à l'aide de la fonctionnalité NVMe multipath intégrée au noyau.
...
Vous pouvez utiliser le support et les fonctionnalités suivants avec la configuration d’hôte NVMe-oF pour Rocky Linux 9.5. Vous devez également vérifier les limites connues avant de commencer le processus de configuration.
...
+
** À partir d'ONTAP 9.12.1, vous pouvez utiliser l'authentification sécurisée en bande pour NVMe-oF avec Rocky Linux 9.5.
** Rocky Linux 9.5 active le multipath NVMe dans le noyau pour les espaces de noms NVMe par défaut, supprimant ainsi le besoin de paramètres explicites.
...
Utilisez la procédure suivante pour valider les versions minimales du logiciel Rocky Linux 9.5 prises en charge.

.Étapes
. Installez Rocky Linux 9.5 sur le serveur. Une fois l'installation terminée, vérifiez que vous utilisez le noyau Rocky Linux 9.5 spécifié :
+
[source, cli]
----
uname -r
