...
summary: 'Comment configurer l"hôte NVMe-oF pour Rocky Linux 8.7 avec ONTAP' 
---
= Configurer Rocky Linux 8.7 avec NVMe-oF pour le stockage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Les hôtes Rocky Linux 8.7 prennent en charge les protocoles NVMe/FC et NVMe/TCP avec l'accès asymétrique aux espaces de noms (ANA). ANA est équivalent à l'accès asymétrique aux unités logiques (ALUA) dans les environnements iSCSI et FCP et est implémenté à l'aide de la fonctionnalité multichemin NVMe intégrée au noyau.
...
Vous pouvez utiliser le support et les fonctionnalités suivants avec la configuration d’hôte NVMe-oF pour Rocky Linux 8.7. Vous devez également vérifier les limites connues avant de commencer le processus de configuration.
...
* Limitations connues :
+
** Le multipath NVMe dans le noyau est désactivé par défaut pour les hôtes Rocky Linux 8.7 NVMe-oF. Par conséquent, vous devez l'activer manuellement.
** Sur les hôtes Rocky Linux 8.7, NVMe/TCP est une fonctionnalité en préversion en raison de problèmes non résolus. Consultez le link:https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/8/html-single/8.7_release_notes/index#technology-preview_file-systems-and-storage["Notes de version de Rocky Linux 8.7"^] pour plus de détails.
...
Utilisez la procédure suivante pour valider les versions minimales du logiciel Rocky Linux 8.7 prises en charge.

.Étapes
. Installez Rocky Linux 8.7 sur le serveur. Une fois l'installation terminée, vérifiez que vous utilisez le noyau Rocky Linux 8.7 spécifié :
+
[source, cli]
----
uname -r
...
La configuration de l'hôte NVMe-oF pour Rocky Linux 8.7 avec stockage ONTAP présente les problèmes connus suivants :
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | Les hôtes NVMe-oF de Rocky Linux 8.7 créent des contrôleurs de découverte persistante en double | Sur les hôtes NVMe over Fabrics (NVMe-of), vous pouvez utiliser la commande nvme Discover -p pour créer des contrôleurs de découverte persistants (CDP). Lorsque cette commande est utilisée, un seul PDC doit être créé par combinaison initiateur-cible. Cependant, si vous exécutez ONTAP 9.10.1 et Rocky Linux 8.7 avec un hôte NVMe-oF, un PDC en double est créé chaque fois que « nvme discover -p » est exécuté. Cela entraîne une utilisation inutile des ressources sur l'hôte et la cible. 
|===