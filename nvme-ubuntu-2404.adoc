...
---
= Configurer Ubuntu 24.04 pour NVMe-oF avec le stockage ONTAP
:allow-uri-read: 
...
Découvrez comment configurer les hôtes NVMe over Fabrics (NVMe-oF) pour Ubuntu 24.04. Pour plus d'informations sur l'assistance et les fonctionnalités, consultez link:nvme-ubuntu-supported-features.html["Prise en charge et fonctionnalités d'Ubuntu ONTAP"].

NVMe-oF avec Ubuntu 24.04 présente les limitations connues suivantes :
...
NOTE: Si les  `hostnqn` chaînes ne correspondent pas, utilisez la commande  `vserver modify` pour mettre à jour la chaîne  `hostnqn` sur le sous-système de système de stockage ONTAP correspondant afin qu'elle corresponde à la chaîne  `hostnqn` provenant de  `/etc/nvme/hostnqn` sur l'hôte.
...
. Vérifiez que vous utilisez le firmware Broadcom `lpfc` recommandé et le pilote inbox :
+
...
.NVMe/TCP
--
NVMe/TCP ne prend pas en charge l'opération de connexion automatique. Vous pouvez découvrir les sous-systèmes et espaces de noms NVMe/TCP en effectuant manuellement les opérations NVMe/TCP  `connect` ou  `connect-all`.
